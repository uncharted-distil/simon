...
###### THIS IS FCHOLLET'S MULTI-GPU CODE, WHICH WILL LATER BE IN .UTILS #######
...
def main(checkpoint, data_count, data_cols, should_train, nb_epoch, null_pct, try_reuse_data, batch_size, execution_config):
    ...
    decay_rate = 0.001*NGPUS / nb_epoch
    ...
    if(should_train):
        start = time.time()
        train_model(batch_size, checkpoint_dir, parallel_model, nb_epoch, data)
        end = time.time()
    ...
    evaluate_model(max_cells, parallel_model, data, encoder)
