...
def main(checkpoint, data_count, data_cols, should_train, nb_epoch, null_pct, try_reuse_data, batch_size, execution_config):
    ...
    # using multi-gpu capabilities
    NGPUS = 2 # specify number of GPUS
    parallel_model = Classifier.multi_gpu_model(model, gpus=NGPUS)
    decay_rate = 0.001*NGPUS / nb_epoch # decay_rate = learning_rate / epochs
    ADAM = keras.optimizers.Adam(lr=0.001*NGPUS, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=decay_rate)
    ...
    if(should_train):
        start = time.time()
        history = Classifier.train_model(batch_size, checkpoint_dir, parallel_model, nb_epoch, data)
        end = time.time()
    ...
    pred_headers = Classifier.evaluate_model(max_cells, parallel_model, data, encoder, p_threshold)
