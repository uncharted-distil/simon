...
def evaluate_model(max_cells, model, data, encoder, p_threshold, ROWDISCREP, ROWDISCREP_MODE, raw_data):
    ...
    print("Time: {0}s \n Time/example : {1}s/ex".format(
        end - start, (end - start) / data.X_test.shape[0]))
    with open('Categories.txt','r') as f:
            Categories = f.read().splitlines()
    Categories = sorted(Categories)
    
    # make sure to discard the "entire column empty" edge case
    if(ROWDISCREP_MODE=='FULL'):
        probabilities[np.all(raw_data.isnull(),axis=0)]=0 # entire pd frame option
    elif(ROWDISCREP_MODE=='TRUNCATED'):
        probabilities[np.all((raw_data=='NaN'),axis=0)]=0
        
    # re-run penny when columns had to be massively truncated/replicated
    if(ROWDISCREP):
        print("SIMON::WARNING::re-running penny")
        start_time_guess = time.time()
        print("Beginning Guessing...")
        category_count = 0
        ordinal_count = 0
        for i in np.arange(len(probabilities)):
            # first, delete unreliable categorical/ordinal predictions
            if(probabilities[i][Categories.index('categorical')]>0):
                probabilities[i][Categories.index('categorical')]=0
            if(probabilities[i][Categories.index('ordinal')]>0):    
                probabilities[i][Categories.index('ordinal')]=0
            if(ROWDISCREP_MODE=='FULL'):
                tmp = guess(raw_data.ix[:,i][raw_data.ix[:,i].notnull()], for_types ='category') # pd frame option, entire column
            elif(ROWDISCREP_MODE=='TRUNCATED'):
                tmp = guess(raw_data[:,i], for_types ='category')
            if tmp[0]=='category':
                category_count += 1
                probabilities[i][Categories.index('categorical')]=1
                if (probabilities[i][Categories.index('int')]>p_threshold) or (probabilities[i][Categories.index('float')]>p_threshold) \
                    or (probabilities[i][Categories.index('datetime')]>p_threshold):
                        ordinal_count += 1
                        probabilities[i][Categories.index('ordinal')]=1
    ...
    # first, find and display the maximum probability/class
    ...
    print(m)
    # next, all probabilities above specified threshold
...
def main(checkpoint, data_count, data_cols, should_train, nb_epoch, null_pct, try_reuse_data, batch_size, execution_config):
    ...
    max_cells = 500
    p_threshold = 0.5
    ROWDISCREP_MODE = 'FULL' # options-'FULL' or 'TRUNCATED' (whether to statiscally analyze all rows, or just max_cells rows, 
                                                              # which can be more consistent with what a human would do) 
...
#    # read test data
#    dataset_name = "pbdb_data"
#    print("BEGINNING TEST/EVALUATION OF MODEL ON DATASET...")
#    raw_data = pd.read_csv('data/test_data/'+dataset_name+'.csv',
#                       dtype='str', header=0, skip_blank_lines=True)
#    
#    
#
#    with open('data/test_data/'+dataset_name+'_header','r',newline='\n') as myfile:
#        reader = csv.reader(myfile, delimiter=',')
#        header = []
#        for row in reader:
#            header.append(row)
    
    # uncomment the below code for unit test option
    dataset_name = "o_38"
    print("BEGINNING UNIT_TEST...")
    raw_data = pd.read_csv('data/unit_test_data/darpa_seeds/'+dataset_name+'/data/trainData.csv',dtype='str')
    ...
    if((nrows > 5*max_cells) or (nrows < 1/5*max_cells)):
        ...
        print("SIMON::WARNING::statistical variable guessing may need to be re-run...")
        ROWDISCREP = True
    ROWDISCREP = False     # uncomment to make code apply the model without re-running the rule-based categorical/ordinal classifier (penny)
    ...
    if(ROWDISCREP_MODE=='FULL'):
        result = evaluate_model(max_cells, model, data, encoder, p_threshold, ROWDISCREP, ROWDISCREP_MODE, raw_data)
    elif(ROWDISCREP_MODE=='TRUNCATED'):
        result = evaluate_model(max_cells, model, data, encoder, p_threshold, ROWDISCREP, ROWDISCREP_MODE, out)
